...
df = df %>% mutate(species = recode(species, mosue = "mouse"))

df_filtered = df %>% filter(r2 >= 0.5 & between(tau,10,1000) == TRUE)

## Add new column to group by brain region (i.e. collapse subregions)

grouped_df = df_filtered %>% mutate(brain_region = ifelse(brain_area %in% c('acc', 'aca', 'dacc', 'scacc'), "ACC",
                                          ifelse(brain_area %in% c('amyg','bla','AMG','centralNuc'), "Amygdala",
                                          ifelse(brain_area %in% c('hc','ca1','ca2','ca3','dg'), "Hippocampus",
                                          ifelse(brain_area %in% c('lai','lapm'), "Insula",
                                          ifelse(brain_area %in% c('mpfc','ila','pl'), "mPFC",
                                          ifelse(brain_area %in% c('ofc','orb','11l','11m','13m','13l','13b','12o'), "OFC",
                                          ifelse(brain_area %in% c('ventralStriatum','pu','cd'), "Striatum",
                                                 "other"))))))))

grouped_df %>% filter(brain_region != 'other') %>% ggplot(aes(x=species,y=tau,color=brain_region,fill=brain_region)) +
...
lmer(tau ~ species * brain_region + (1|fr), 
     data = grouped_df %>% filter(brain_region != 'other')) %>% summary()

## Separate by brain region, plot, GLMs

acc = grouped_df %>% filter(brain_region == 'ACC')
amyg = grouped_df %>% filter(brain_region == 'Amygdala')
hc = grouped_df %>% filter(brain_region == 'Hippocampus')
insula = grouped_df %>% filter(brain_region == 'Insula')
mpfc = grouped_df %>% filter(brain_region == 'mPFC')
ofc = grouped_df %>% filter(brain_region == 'OFC')
str = grouped_df %>% filter(brain_region == 'Striatum')
...
acc_emm = lmer(tau ~ species + (1|fr), data = acc) %>% emmeans(~ species)
pwpp(acc_emm, comparisions = TRUE) + ggtitle('ACC')

lmer(tau ~ species + brain_area + (1|fr), data = acc) %>% summary()
...
amyg_emm = lmer(tau ~ species + (1|fr), data = amyg) %>% emmeans(~ species)
pwpp(amyg_emm, comparisons = TRUE) + ggtitle('Amygdala')

lmer(tau ~ species + brain_area + (1|fr), data = amyg) %>% summary()
...
hc_emm = lmer(tau ~ species + (1|fr), data = hc) %>% emmeans(~ species)
pwpp(hc_emm, comparisons = TRUE) + ggtitle('Hippocampus')

lmer(tau ~ species + brain_area + (1|fr), data = hc) %>% summary()
...
mpfc_emm = lmer(tau ~ species + (1|fr), data = mpfc) %>% emmeans(~ species)
pwpp(mpfc_emm, comparisons = TRUE) + ggtitle('mPFC')

lmer(tau ~ species + brain_area + (1|fr), data = mpfc) %>% summary()
lmer(tau ~ species + brain_area + (1|fr), data = mpfc) %>% emmeans(pairwise ~ species * brain_area)
...
ofc_emm = lmer(tau ~ species + (1|fr), data = ofc) %>% emmeans(~ species)
pwpp(ofc_emm, comparisons = TRUE) + ggtitle('OFC')

lmer(tau ~ species + brain_area + (1|fr), data = ofc) %>% summary()
lmer(tau ~ species + brain_area + (1|fr), data = ofc) %>% emmeans(pairwise ~ species * brain_area)
...
ofc_ins = grouped_df %>% filter(brain_region == 'OFC' | brain_region == 'Insula')

ofc_ins %>% ggplot(aes(x=species,y=tau,color=brain_region,fill=brain_region)) +
  geom_flat_violin(position=position_nudge(x=0.2,y=0),adjust=2,alpha=0.5) + 
  geom_point(position=position_jitter(width=0.15),size=0.25) + coord_flip() +
...
grouped_df %>% filter(species == 'mouse') %>% ggplot(aes(x=brain_region,y=tau,color=dataset,fill=dataset)) +
  geom_flat_violin(position=position_nudge(x=0.2,y=0),adjust=2,alpha=0.5) + 
  geom_point(position=position_jitter(width=0.15),size=0.25) + coord_flip() +
...
grouped_df %>% filter(species == 'rat') %>% ggplot(aes(x=brain_region,y=tau,color=dataset,fill=dataset)) +
  geom_flat_violin(position=position_nudge(x=0.2,y=0),adjust=2,alpha=0.5) + 
  geom_point(position=position_jitter(width=0.15),size=0.25) + coord_flip() +
...
grouped_df %>% filter(species == 'monkey') %>% ggplot(aes(x=brain_region,y=tau,color=dataset,fill=dataset)) +
  geom_flat_violin(position=position_nudge(x=0.2,y=0),adjust=2,alpha=0.5) + 
  geom_point(position=position_jitter(width=0.15),size=0.25) + coord_flip() +
...
grouped_df %>% filter(species == 'human') %>% ggplot(aes(x=brain_region,y=tau,color=dataset,fill=dataset)) +
  geom_flat_violin(position=position_nudge(x=0.2,y=0),adjust=2,alpha=0.5) + 
  geom_point(position=position_jitter(width=0.15),size=0.25) + coord_flip() +
