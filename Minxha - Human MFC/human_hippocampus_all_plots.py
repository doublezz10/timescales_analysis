...
for unit in range(len(spikes)):
        ...
        if np.isnan(one_autocorrelation).any() == True:
            ...
            hc_taus.append(pars[1])
            
            all_means_hc.append(y_m)
...
all_means_hc = np.vstack(all_means_hc)

mean_hc = np.mean(all_means_hc,axis=0)
sd_hc = np.std(all_means_hc,axis=0)
se_hc = sd_hc/np.sqrt(len(mean_hc))

def func(x,a,tau,b):
    return a*((np.exp(-x/tau))+b)

pars_hc,cov = curve_fit(func,x_m,mean_hc,p0=[1,100,1],bounds=((0,np.inf)))

plt.plot(x_m,mean_hc,label='original data')
plt.plot(x_m,func(x_m,*pars_hc),label='fit curve')
plt.legend(loc='upper right')
plt.xlabel('lag (ms)')
plt.ylabel('mean autocorrelation')
plt.title('Mean of all human hippocampus units')
plt.text(710,0.03,'tau = %i' %pars_hc[1])
plt.show()
...
plt.xlabel('tau')
plt.ylabel('count')
plt.title('%i human hippocampus units' %len(hc_taus))
plt.show()