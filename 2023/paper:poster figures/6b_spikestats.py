...
plt.show()
# %% how much variance is explained by all the predictors

model = smf.ols('tau ~ species + brain_region + z_n_spikes + z_mean_fr + z_mean_norm_fr + z_fano + z_prop_pause + z_prop_burst',data=data)
res = model.fit()

print(res.summary())

model2 = smf.ols('tau ~ species + brain_region',data=data)
res2 = model2.fit()
...
model = smf.ols('lat ~ species + brain_region + z_n_spikes + z_mean_fr + z_mean_norm_fr + z_fano + z_prop_pause + z_prop_burst',data=data)
res = model.fit()

print(res.summary())

model2 = smf.ols('lat ~ species + brain_region',data=data)
res2 = model2.fit()
...
#%% what about just the significant ones
...
model = smf.ols('lat ~ species + brain_region + z_n_spikes + z_mean_fr + z_fano',data=data)
res = model.fit()
