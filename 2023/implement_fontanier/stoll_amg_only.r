...
data = readMat('/Users/zachz/Library/Mobile Documents/com~apple~CloudDocs/Timescales Data/stoll_AMG.mat')

spiketimes = data$spikes
cell_info = data$cell.info.full
  ...
  if(!is.na(dip)){
    ...
    for (fit_iter in 1:nrow(start)) {
      ...
      first_peak_to_dip_fits = rbind(first_peak_to_dip_fits,
                                     exp_fit_fun(peak_to_dip,start))
    }
    first_peak_to_dip_fits$fit = 'dip1'
    first_peak_to_dip_fits$dof = nrow(first_peak_to_dip_fits)
  } else {
  ...
  if(!is.na(peak.second)){
    ...
    for (fit_iter in 1:nrow(start)){
      ...
      second_peak_to_end_fits = rbind(second_peak_to_end_fits,
                                      exp_fit_fun(second_peak_to_end,start))
    }
    second_peak_to_end_fits$fit = 'dip2'
    second_peak_to_end_fits$dof = nrow(second_peak_to_end_fits)
