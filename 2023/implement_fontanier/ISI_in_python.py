...
data = spio.loadmat('/Users/zachz/Library/Mobile Documents/com~apple~CloudDocs/Timescales Data/fontanier_mcc.mat',squeeze_me=True,simplify_cells=True)
...
for unit in range(len(spiketimes)):
    ...
    for lag in range(100):
        ...
        isi = np.diff(spiketimeseries,n=lag)
        isi = isi[isi>0]
        isi = isi[isi<1]
    ...
    auto['countsn'] = auto['counts']/auto_total * 1/auto_width

    # smooth

    auto['smooth_countsn'] = savgol_filter(auto['countsn'],window_length=100,polyorder=2)
    ...
    ISIs['countsn'] = ISIs['counts']/ISIs_tot * 1/auto_width
    ...
    def curve_fit_func(auto,start):
        ...
        try:
            ...
            df_fit = pd.DataFrame({'tau':[tau],'tau_error':[tau_err],'A':[a],'B':[b],'fit_err':[fit_err]})           
        except:
            
            df_fit = pd.DataFrame({'tau':[np.nan],'tau_error':[np.nan],'A':[np.nan],'B':[np.nan],'fit_err':[np.nan]})
    ...
    best_fits = pd.DataFrame(all_fits[all_fits.rmse == np.nanmin(all_fits.rmse)])
    
    if len(best_fits) > 1:
        
        best_fit = pd.DataFrame(best_fits.iloc[0]).T
    
    all_all = pd.concat((all_all,best_fit))

all_all.to_csv('mcc_python_isi.csv')
