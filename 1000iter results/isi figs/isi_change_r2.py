...
for r2 in [0.5,0.55,0.6,0.65,0.7,0.75,0.8,0.85,0.9,0.95]:
    ...
    for species in listofspecies:
        ...
        for region in this_species.brain_region.unique():
            ...
            corrs.append((species,region,corr,p,r2,n,np.mean(fred_taus),np.mean(zach_taus)))
            
corrs = pd.DataFrame(corrs,columns=['species','brain_region','corr','pval','r2','n','mean_f','mean_z'])
...
ax1 = sns.scatterplot(data=mouse_amyg,x='r2',y='n',color='green')

ax1.set_xlabel('R$^2$ thresh')

ax2 = ax1.twinx()
ax2 = sns.scatterplot(data=mouse_amyg,x='r2',y='corr')

mean_corr = np.mean(corrs[corrs.r2==0.5]['corr'])
...
sns.scatterplot(data=mouse_amyg,x='r2',y='mean_f',color='blue',label='isi')
sns.scatterplot(data=mouse_amyg,x='r2',y='mean_z',color='green',label='item')

mean_taus = corrs[corrs.r2==0.5].groupby('species',as_index=False).mean()
