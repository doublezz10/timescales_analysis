...
data = data %>% group_by(species,dataset,brain_area) %>% summarize_at(vars('tau','fr','n_units'),mean)
...
grouped_data = data %>% mutate(brain_region = ifelse(brain_area %in% c('acc', 'aca', 'dACC','mcc'), "ACC",
                                              ...
                                              ifelse(brain_area %in% c('hc','ca1','ca2','ca3','dg','hippocampus'), "Hippocampus",
...
grouped_data %>% 
  ...
  summarise(mean_tau = mean(tau), sd = sd(tau), tau = tau) %>%
  ggplot(aes(x=reorder(brain_region,mean_tau),y=tau,color=species,fill=species)) +
  geom_point(size=3) + geom_line(aes(x=reorder(brain_region,mean_tau),y=mean_tau,group=species))+
  xlab('Brain Region') +
