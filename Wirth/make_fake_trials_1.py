...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 15 15:41:49 2021
...
hc = spio.loadmat('/Users/zachz/Dropbox/Timescales across species/Spiketimes only/Wirth - monkey hippocampus/wirth_hippocampus.mat',simplify_cells=True)
...
wirth_hc1_all_means = []
wirth_hc1_taus = []
wirth_hc1_failed_fits = []

wirth_hc1_failed_autocorr = []
wirth_hc1_no_spikes_in_a_bin = []
wirth_hc1_low_fr = []

wirth_hc1_avg_fr = []
wirth_hc1_correlation_matrices = []
wirth_hc1_r2s = []
...
for unit in range(len(spikes)):
    ...
    try:
        ...
        if np.isnan(one_autocorrelation).any() == True:
            ...
            wirth_hc1_failed_autocorr.append(unit) # skip this unit if any autocorrelation fails
    
        elif [summed_spikes_per_bin[bin] == 0 for bin in range(len(summed_spikes_per_bin))] == True:
    
            wirth_hc1_no_spikes_in_a_bin.append(unit) # skip this unit if any bin doesn't have spikes
    
        elif np.sum(summed_spikes_per_bin) < 1:
    
            wirth_hc1_low_fr.append(unit) # skip this unit if avg firing rate across all trials is < 1
            ...
            wirth_hc1_correlation_matrices.append(correlation_matrix)
            ...
            try:
                ...
                wirth_hc1_failed_fits.append(unit)
    
            r2 = (np.corrcoef(y_m[first_neg_diff:],func(x_m[first_neg_diff:],*pars)))[0,1]**2
            wirth_hc1_r2s.append(r2)
    
            wirth_hc1_taus.append(pars[1])
            wirth_hc1_avg_fr.append(np.sum(summed_spikes_per_bin)/trials)
    
            wirth_hc1_all_means.append(y_m)
            ...
            all_data_wirth_hc1.append(('wirth1','monkey','hippocampus',unit,pars[1],np.sum(summed_spikes_per_bin)/trials,r2,pars[0],pars[2]))
...
with open('/Users/zachz/Documents/timescales_analysis/results.csv','a') as out:
    ...
    for row in all_data_wirth_hc1:
        csv_out.writerow(row)
        
#%% How many units got filtered?

wirth_hc1_bad_units = len(wirth_hc1_failed_autocorr) + len(wirth_hc1_no_spikes_in_a_bin) + len(wirth_hc1_low_fr)

print('%i units were filtered out' %wirth_hc1_bad_units)
print('out of %i total units' %len(spikes))

#%% Take mean of all units

wirth_hc1_all_means = np.vstack(wirth_hc1_all_means)

wirth_hc1_mean = np.mean(wirth_hc1_all_means,axis=0)
wirth_hc1_sd = np.std(wirth_hc1_all_means,axis=0)
wirth_hc1_se = wirth_hc1_sd/np.sqrt(len(wirth_hc1_mean))

wirth_hc1_mean_fr = np.mean(wirth_hc1_avg_fr)

def func(x,a,tau,b):
    return a*((np.exp(-x/tau))+b)

mean_diff = np.diff(wirth_hc1_mean)
...
wirth_hc1_pars,cov = curve_fit(func,x_m[first_neg_mean_diff:],wirth_hc1_mean[first_neg_mean_diff:],p0=[1,100,1],bounds=((0,np.inf)))

plt.plot(x_m,wirth_hc1_mean,label='original data')
plt.plot(x_m[first_neg_mean_diff:],func(x_m[first_neg_mean_diff:],*wirth_hc1_pars),label='fit curve')
plt.legend(loc='upper right')
plt.xlabel('lag (ms)')
plt.ylabel('autocorrelation')
plt.title('Mean of all monkey hippocampus units \n Wirth')
plt.text(710,0.075,'tau = %i ms \n fr = %.2f hz \n n = %i' % (wirth_hc1_pars[1],wirth_hc1_mean_fr,len(wirth_hc1_taus)))
plt.show()

a_population_wirth_hc1 = (('wirth','hc',wirth_hc1_pars[1],wirth_hc1_mean_fr,len(wirth_hc1_taus)))

#%% Add error bars

plt.errorbar(x_m, wirth_hc1_mean, yerr=wirth_hc1_se, label='data +/- se')
plt.plot(x_m[first_neg_mean_diff:],func(x_m[first_neg_mean_diff:],*wirth_hc1_pars),label='fit curve')
plt.legend(loc='upper right')
plt.xlabel('lag (ms)')
plt.ylabel('autocorrelation')
...
plt.hist(wirth_hc1_taus,bins=bins, weights=np.zeros_like(wirth_hc1_taus) + 1. / len(wirth_hc1_taus))
plt.axvline(wirth_hc1_pars[1],color='r',linestyle='dashed',linewidth=1)
plt.xlabel('tau (ms)')
plt.ylabel('proportion')
plt.xscale('log')
plt.title('%i Monkey hippocapmus units \n Wirth' %len(wirth_hc1_taus))
plt.show()

#%% Correlation matrix

wirth_hc1_mean_matrix = np.mean(wirth_hc1_correlation_matrices,axis=0)

plt.imshow(wirth_hc1_mean_matrix)
plt.tight_layout()
plt.title('Wirth monkey hippocampus')
plt.xlabel('lag (ms)')
plt.ylabel('lag (ms)')
