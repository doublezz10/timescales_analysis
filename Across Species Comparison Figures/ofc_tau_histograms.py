...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan 22 15:48:56 2021
...
n,x,_ = plt.hist(feierstein, bins = 10**np.arange(0,4,0.1), histtype=u'step' ,weights=np.zeros_like(feierstein) + 1/len(feierstein))
bin_centers = 0.5*(x[1:]+x[:-1])
plt.plot(bin_centers,n,'o-',label='Feierstein (rat)')
...
n,x,_ = plt.hist(kepecs, bins = 10**np.arange(0,4,0.1), histtype=u'step' ,weights=np.zeros_like(kepecs) + 1/len(kepecs))
bin_centers = 0.5*(x[1:]+x[:-1])
plt.plot(bin_centers,n,'o-',label='Kepecs (rat)')
