...
for unit in range(len(spikes)):
        ...
        if np.isnan(one_autocorrelation).any() == True:
            ...
            all_data_minxha_dacc.append(('minxha','human','dACC',unit,pars[1],np.sum(summed_spikes_per_bin)/trials,r2,pars[0],pars[2]))

with open('/Users/zachz/Documents/timescales_analysis/results.csv','a') as out:
    csv_out=csv.writer(out)
    for row in all_data_minxha_dacc:
...
a_population_minxha_dacc = (('minxha','dacc',minxha_dacc_pars[1],minxha_dacc_mean_fr,len(minxha_dacc_taus)))
...
plt.axvline(minxha_dacc_pars[1],color='r',linestyle='dashed',linewidth=1)
plt.xlabel('tau (ms)')
