...
for unit in range(len(spikes)):
        ...
        if np.isnan(one_autocorrelation).any() == True:
            ...
            all_data_minxha_presma.append(('minxha','human','preSMA',unit,pars[1],np.sum(summed_spikes_per_bin)/trials,r2,pars[0],pars[2]))

with open('/Users/zachz/Documents/timescales_analysis/results.csv','a') as out:
    csv_out=csv.writer(out)
    for row in all_data_minxha_presma:
...
a_population_minxha_presma = (('minxha','preSMA',minxha_presma_pars[1],minxha_presma_mean_fr,len(minxha_presma_taus)))
...
plt.axvline(minxha_presma_pars[1],color='r',linestyle='dashed',linewidth=1)
plt.xlabel('tau (ms)')
