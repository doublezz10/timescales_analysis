...
for unit in range(len(spikes)):
        ...
        if np.isnan(one_autocorrelation).any() == True:
            ...
            all_data_minxha_amyg.append(('minxha','human','amygdala',unit,pars[1],np.sum(summed_spikes_per_bin)/trials,r2,pars[0],pars[2]))

with open('/Users/zachz/Documents/timescales_analysis/results.csv','a') as out:
    csv_out=csv.writer(out)
    for row in all_data_minxha_amyg:
...
best_tau_mean = tau_test_mean[np.argmax(r2_test_mean)]

plt.plot(x_m,minxha_amyg_mean,label='original data')
plt.plot(x_m[first_neg_mean_diff:],func(x_m[first_neg_mean_diff:],*all_mean_pars[np.argmax(r2_test_mean)]),label='fit curve')
...
a_population_minxha_amyg = (('minxha','amyg',minxha_amyg_pars[1],minxha_amyg_mean_fr,len(minxha_amyg_taus)))
