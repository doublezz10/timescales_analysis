...
for unit in range(len(spikes)):
        ...
        #     acc_no_spikes_in_a_bin_monkey.append(unit)
            ...
            try:
                ...
                acc_failed_fits.append(unit)
            
            acc_taus_monkey.append(pars[1])
            
            all_means_acc_monkey.append(y_m)
...
all_means_acc_monkey = np.vstack(all_means_acc_monkey)

mean_acc = np.mean(all_means_acc_monkey,axis=0)
sd_acc = np.std(all_means_acc_monkey,axis=0)
...
plt.hist(acc_taus_monkey)
plt.xlabel('tau')
plt.ylabel('count')
plt.title('%i monkey acc units' %len(acc_taus_monkey))
plt.show()
...
# units = np.size(all_means_acc_monkey,0)

# for unit in range(units):
    
#     first_diff_acc.append(all_means_acc_monkey[unit,0]-all_means_acc_monkey[unit,1])
#     second_diff_acc.append(all_means_acc_monkey[unit,1]-all_means_acc_monkey[unit,2])
...
#     axs[0].plot(x_m,all_means_acc_monkey[neg_first_diff[dec_unit]])
...
#    axs[1].plot(x_m,all_means_acc_monkey[pos_first_diff[inc_unit]])
