...
for unit in range(len(spikes)):
        ...
        #     dlpfc_no_spikes_in_a_bin_monkey.append(unit)
            ...
            dlpfc_low_fr_monkey.append(unit)
            ...
            try:
                ...
                dlpfc_failed_fits.append(unit)
            
            dlpfc_taus_monkey.append(pars[1])
            
            all_means_dlpfc_monkey.append(y_m)
...
all_means_dlpfc_monkey = np.vstack(all_means_dlpfc_monkey)

mean_dlpfc = np.mean(all_means_dlpfc_monkey,axis=0)
sd_dlpfc = np.std(all_means_dlpfc_monkey,axis=0)
...
pars_dlpfc,cov = curve_fit(func,x_m[first_neg_mean_diff:],mean_dlpfc[first_neg_mean_diff:],p0=[1,100,1],bounds=((0,np.inf)))

plt.plot(x_m,mean_dlpfc,label='original data')
plt.plot(x_m[first_neg_mean_diff:],func(x_m[first_neg_mean_diff:],*pars_dlpfc),label='fit curve')
plt.legend(loc='upper right')
plt.xlabel('lag (ms)')
plt.ylabel('mean autocorrelation')
plt.title('Mean of all monkey dlpfc units')
plt.text(100,0.095,'tau = %i' %pars_dlpfc[1])
plt.show()

#%% Histogram of taus

plt.hist(dlpfc_taus_monkey)
plt.xlabel('tau')
plt.ylabel('count')
plt.title('%i monkey dlpfc units' %len(dlpfc_taus_monkey))
plt.show()
...
# units = np.size(all_means_dlpfc_monkey,0)

# for unit in range(units):
    
#     first_diff_dlpfc.append(all_means_dlpfc_monkey[unit,0]-all_means_dlpfc_monkey[unit,1])
#     second_diff_dlpfc.append(all_means_dlpfc_monkey[unit,1]-all_means_dlpfc_monkey[unit,2])
    
# plt.hist(first_diff_dlpfc,label='first diff')
# plt.hist(second_diff_dlpfc,label='second diff')
# plt.ylabel('count')
# plt.title('dlpfc')
# plt.legend()
# plt.show()
...
#     axs[0].plot(x_m,all_means_dlpfc_monkey[neg_first_diff[dec_unit]])
    
# axs[0].set_title('%i dlpfc units with \n initial increase' %len(neg_first_diff))
# axs[0].set_ylabel('autocorrelation')
# axs[0].set_xlabel('lag (ms)')

# for inc_unit in range(len(pos_first_diff)):
    
#    axs[1].plot(x_m,all_means_dlpfc_monkey[pos_first_diff[inc_unit]])
    
# axs[1].set_title('%i dlpfc units with \n initial decrease' % len(pos_first_diff))
