...
all_means_unknown_monkey = []
unknown_taus_monkey = []
unknown_failed_fits = []

unknown_no_autocorr_monkey = []
unknown_no_spikes_in_a_bin_monkey = []
unknown_low_fr_monkey = []
...
for unit in range(len(spikes)):
        ...
        if np.isnan(one_autocorrelation).any() == True:
            ...
            unknown_no_autocorr_monkey.append(unit)
            
        # elif [summed_spikes_per_bin[bin] == 0 for bin in range(len(summed_spikes_per_bin))]:
            
        #     unknown_no_spikes_in_a_bin_monkey.append(unit)
            
        elif np.sum(summed_spikes_per_bin) < 1:
            
            unknown_low_fr_monkey.append(unit)
            ...
            try:
                ...
                unknown_failed_fits.append(unit)
            
            unknown_taus_monkey.append(pars[1])
            
            all_means_unknown_monkey.append(y_m)
...
all_means_unknown_monkey = np.vstack(all_means_unknown_monkey)

mean_unknown = np.mean(all_means_unknown_monkey,axis=0)
sd_unknown = np.std(all_means_unknown_monkey,axis=0)
...
plt.hist(unknown_taus_monkey)
plt.xlabel('tau')
plt.ylabel('count')
plt.title('%i monkey unknown units' %len(unknown_taus_monkey))
plt.show()
...
# units = np.size(all_means_unknown_monkey,0)

# for unit in range(units):
    
#     first_diff_unknown.append(all_means_unknown_monkey[unit,0]-all_means_unknown_monkey[unit,1])
#     second_diff_unknown.append(all_means_unknown_monkey[unit,1]-all_means_unknown_monkey[unit,2])
    
# plt.hist(first_diff_unknown,label='first diff')
# plt.hist(second_diff_unknown,label='second diff')
# plt.ylabel('count')
# plt.title('unknown')
# plt.legend()
# plt.show()
...
#     axs[0].plot(x_m,all_means_unknown_monkey[neg_first_diff[dec_unit]])
    
# axs[0].set_title('%i unknown units with \n initial increase' %len(neg_first_diff))
# axs[0].set_ylabel('autocorrelation')
# axs[0].set_xlabel('lag (ms)')

# for inc_unit in range(len(pos_first_diff)):
    
#    axs[1].plot(x_m,all_means_unknown_monkey[pos_first_diff[inc_unit]])
    
# axs[1].set_title('%i unknown units with \n initial decrease' % len(pos_first_diff))
