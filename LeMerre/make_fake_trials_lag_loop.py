...
for unit in range(len(spikes)):
    ...
    if np.isnan(one_autocorrelation).any() == True:
        ...
        all_data_lemerre_mpfc.append(('lemerre','rat','mPFC',unit,pars[1],np.sum(summed_spikes_per_bin)/trials,r2,pars[0],pars[2]))

with open('/Users/zachz/Documents/timescales_analysis/results.csv','a') as out:
    csv_out=csv.writer(out)
    for row in all_data_lemerre_mpfc:
...
a_lemerre = (('lemerre','mPFC',lemerre_mpfc_pars[1],lemerre_mpfc_mean_fr,len(lemerre_mpfc_taus)))
