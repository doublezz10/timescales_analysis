...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 16 17:30:33 2021
...
vlpfc = spio.loadmat('/Users/zachz/Dropbox/Timescales across species/Spiketimes only/Stoll - monkey1/stoll_vlPFC.mat',simplify_cells=True)

spikes = vlpfc['spikes']

try:
   cell_info = vlpfc['cell_info']
except KeyError:
   pass

#%% Begin working!

all_data_stoll_vlpfc = []

stoll_vlpfc_all_means = []
stoll_vlpfc_taus = []
stoll_vlpfc_failed_fits = []

stoll_vlpfc_failed_autocorr = []
stoll_vlpfc_no_spikes_in_a_bin = []
stoll_vlpfc_low_fr = []

stoll_vlpfc_avg_fr = []
stoll_vlpfc_correlation_matrices = []
stoll_vlpfc_r2s = []
...
for unit in range(len(spikes)):
    ...
    if np.isnan(one_autocorrelation).any() == True:
        ...
        stoll_vlpfc_failed_autocorr.append(unit) # skip this unit if any autocorrelation fails

    elif [summed_spikes_per_bin[bin] == 0 for bin in range(len(summed_spikes_per_bin))] == True:

        stoll_vlpfc_no_spikes_in_a_bin.append(unit) # skip this unit if any bin doesn't have spikes

    elif np.sum(summed_spikes_per_bin) < 1:

        stoll_vlpfc_low_fr.append(unit) # skip this unit if avg firing rate across all trials is < 1
        ...
        stoll_vlpfc_correlation_matrices.append(correlation_matrix)
        ...
        try:
            ...
            stoll_vlpfc_failed_fits.append(unit)

        r2 = (np.corrcoef(y_m[first_neg_diff:],func(x_m[first_neg_diff:],*pars)))[0,1]**2
        stoll_vlpfc_r2s.append(r2)

        stoll_vlpfc_taus.append(pars[1])
        stoll_vlpfc_avg_fr.append(np.sum(summed_spikes_per_bin)/trials)

        stoll_vlpfc_all_means.append(y_m)

        # plt.plot(x_m,y_m,'ro',label='original data')
        # plt.plot(x_m[first_neg_diff:],func(x_m[first_neg_diff:],*pars),label='fit')
        # plt.xlabel('lag (ms)')
        # plt.ylabel('mean autocorrelation')
        # plt.title('Monkey vlpfc %i' %unit)
        # plt.legend()
        # plt.show()
        
        #%% Add data to 'all_data'
        
        all_data_stoll_vlpfc.append(('stoll','monkey','vlPFC',unit,pars[1],np.sum(summed_spikes_per_bin)/trials,r2,pars[0],pars[2]))

with open('/Users/zachz/Documents/timescales_analysis/results.csv','a') as out:
    csv_out=csv.writer(out)
    for row in all_data_stoll_vlpfc:
        csv_out.writerow(row)
        
#%% How many units got filtered?

stoll_vlpfc_bad_units = len(stoll_vlpfc_failed_autocorr) + len(stoll_vlpfc_no_spikes_in_a_bin) + len(stoll_vlpfc_low_fr)

print('%i units were filtered out' %stoll_vlpfc_bad_units)
print('out of %i total units' %len(spikes))

#%% Take mean of all units

stoll_vlpfc_all_means = np.vstack(stoll_vlpfc_all_means)

stoll_vlpfc_mean = np.mean(stoll_vlpfc_all_means,axis=0)
stoll_vlpfc_sd = np.std(stoll_vlpfc_all_means,axis=0)
stoll_vlpfc_se = stoll_vlpfc_sd/np.sqrt(len(stoll_vlpfc_mean))

stoll_vlpfc_mean_fr = np.mean(stoll_vlpfc_avg_fr)

def func(x,a,tau,b):
    return a*((np.exp(-x/tau))+b)

mean_diff = np.diff(stoll_vlpfc_mean)
...
stoll_vlpfc_pars,cov = curve_fit(func,x_m[first_neg_mean_diff:],stoll_vlpfc_mean[first_neg_mean_diff:],p0=[1,100,1],bounds=((0,np.inf)))

plt.plot(x_m,stoll_vlpfc_mean,label='original data')
plt.plot(x_m[first_neg_mean_diff:],func(x_m[first_neg_mean_diff:],*stoll_vlpfc_pars),label='fit curve')
plt.legend(loc='upper right')
plt.xlabel('lag (ms)')
plt.ylabel('autocorrelation')
plt.title('Mean of all monkey vlPFC units \n Stoll')
plt.text(710,0.075,'tau = %i ms \n fr = %.2f hz \n n = %i' % (stoll_vlpfc_pars[1],stoll_vlpfc_mean_fr,len(stoll_vlpfc_taus)))
plt.show()


#%% Add error bars

plt.errorbar(x_m, stoll_vlpfc_mean, yerr=stoll_vlpfc_se, label='data +/- se')
plt.plot(x_m[first_neg_mean_diff:],func(x_m[first_neg_mean_diff:],*stoll_vlpfc_pars),label='fit curve')
plt.legend(loc='upper right')
plt.xlabel('lag (ms)')
plt.ylabel('autocorrelation')
...
plt.hist(stoll_vlpfc_taus,bins=bins, weights=np.zeros_like(stoll_vlpfc_taus) + 1. / len(stoll_vlpfc_taus))
plt.axvline(stoll_vlpfc_pars[1],color='r',linestyle='dashed',linewidth=1)
plt.xlabel('tau (ms)')
plt.ylabel('proportion')
plt.xscale('log')
plt.title('%i monkey vlPFC units \n Stoll' %len(stoll_vlpfc_taus))
plt.show()

#%% Correlation matrix

stoll_vlpfc_mean_matrix = np.mean(stoll_vlpfc_correlation_matrices,axis=0)

plt.imshow(stoll_vlpfc_mean_matrix)
plt.tight_layout()
plt.title('Stoll monkey vlpfc')
plt.xlabel('lag (ms)')
plt.ylabel('lag (ms)')
