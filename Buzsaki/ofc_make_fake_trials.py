...
for unit in range(len(spikes)):
    ...
    if np.isnan(one_autocorrelation).any() == True:
        ...
        all_data_buzsaki_ofc.append(('buzsaki','rat','ofc',unit,pars[1],np.sum(summed_spikes_per_bin)/trials,r2,pars[0],pars[2]))

with open('/Users/zachz/Documents/timescales_analysis/results.csv','a') as out:
    csv_out=csv.writer(out)
    for row in all_data_buzsaki_ofc:
...
a_population_buzsaki_ofc = (('buzsaki','ofc',buzsaki_ofc_pars[1],buzsaki_ofc_mean_fr,len(buzsaki_ofc_taus)))
...
plt.axvline(buzsaki_ofc_pars[1],color='r',linestyle='dashed',linewidth=1)
plt.xlabel('tau (ms)')
