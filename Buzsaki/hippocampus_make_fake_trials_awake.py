...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 23 17:24:31 2021
...
buzsaki_hc_all_means = []
buzsaki_hc_taus = []
buzsaki_hc_failed_fits = []

buzsaki_hc_failed_autocorr = []
buzsaki_hc_no_spikes_in_a_bin = []
buzsaki_hc_low_fr = []

buzsaki_hc_avg_fr = []
buzsaki_hc_correlation_matrices = []

for unit in range(len(spikes)):

    unit_spikes = np.array(spikes[unit])
    
    if unit_spikes == []:
        
        pass
    
    elif type(unit_spikes) == float():
        ...
        try:
            ...
            if np.isnan(one_autocorrelation).any() == True:
                ...
                buzsaki_hc_failed_autocorr.append(unit) # skip this unit if any autocorrelation fails
        
            elif [summed_spikes_per_bin[bin] == 0 for bin in range(len(summed_spikes_per_bin))] == True:
        
                buzsaki_hc_no_spikes_in_a_bin.append(unit) # skip this unit if any bin doesn't have spikes
        
            elif np.sum(summed_spikes_per_bin) < 1:
        
                buzsaki_hc_low_fr.append(unit) # skip this unit if avg firing rate across all trials is < 1
                ...
                buzsaki_hc_correlation_matrices.append(correlation_matrix)
                ...
                try:
                    ...
                    buzsaki_hc_failed_fits.append(unit)
                    
                r2 = (np.corrcoef(y_m[first_neg_diff:],func(x_m[first_neg_diff:],*pars)))[0,1]**2
        
                buzsaki_hc_taus.append(pars[1])
                buzsaki_hc_avg_fr.append(np.sum(summed_spikes_per_bin)/trials)
        
                buzsaki_hc_all_means.append(y_m)
                ...
                all_data_buzsaki_hc.append(('buzsaki','rat','bla',unit,pars[1],np.sum(summed_spikes_per_bin)/trials,r2,pars[0],pars[2]))
...
buzsaki_hc_bad_units = len(buzsaki_hc_failed_autocorr) + len(buzsaki_hc_no_spikes_in_a_bin) + len(buzsaki_hc_low_fr)

print('%i units were filtered out' %buzsaki_hc_bad_units)
print('out of %i total units' %len(spikes))

#%% Take mean of all units

buzsaki_hc_all_means = np.vstack(buzsaki_hc_all_means)

buzsaki_hc_mean = np.mean(buzsaki_hc_all_means,axis=0)
buzsaki_hc_sd = np.std(buzsaki_hc_all_means,axis=0)
buzsaki_hc_se = buzsaki_hc_sd/np.sqrt(len(buzsaki_hc_mean))

buzsaki_hc_mean_fr = np.mean(buzsaki_hc_avg_fr)

def func(x,a,tau,b):
    return a*((np.exp(-x/tau))+b)

mean_diff = np.diff(buzsaki_hc_mean)
...
buzsaki_hc_pars,cov = curve_fit(func,x_m[first_neg_mean_diff:],buzsaki_hc_mean[first_neg_mean_diff:],p0=[1,100,1],bounds=((0,np.inf)))

plt.plot(x_m,buzsaki_hc_mean,label='original data')
plt.plot(x_m[first_neg_mean_diff:],func(x_m[first_neg_mean_diff:],*buzsaki_hc_pars),label='fit curve')
...
a_population_buzsaki_hc = (('buzsaki','hippocampus',buzsaki_hc_pars[1],buzsaki_hc_mean_fr,len(buzsaki_hc_taus)))


#%% Add error bars

plt.errorbar(x_m, buzsaki_hc_mean, yerr=buzsaki_hc_se, label='data +/- se')
plt.plot(x_m[first_neg_mean_diff:],func(x_m[first_neg_mean_diff:],*buzsaki_hc_pars),label='fit curve')
plt.legend(loc='upper right')
plt.xlabel('lag (ms)')
plt.ylabel('autocorrelation')
...
plt.hist(buzsaki_hc_taus,bins=bins, weights=np.zeros_like(buzsaki_hc_taus) + 1. / len(buzsaki_hc_taus))
plt.axvline(buzsaki_hc_pars[1],color='r',linestyle='dashed',linewidth=1)
plt.xlabel('tau (ms)')
plt.ylabel('proportion')
plt.xscale('log')
plt.title('%i Rat hippocampus units \n Buzsaki, awake' %len(buzsaki_hc_taus))
plt.show()

#%% Correlation matrix

buzsaki_hc_mean_matrix = np.mean(buzsaki_hc_correlation_matrices,axis=0)

plt.imshow(buzsaki_hc_mean_matrix)
plt.tight_layout()
plt.title('Buzsaki Rat hippocampus')
plt.xlabel('lag (ms)')
plt.ylabel('lag (ms)')
