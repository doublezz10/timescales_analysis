...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 23 17:27:49 2021
...
all_data_buzsaki_amy = []

buzsaki_amy_all_means = []
buzsaki_amy_taus = []
buzsaki_amy_failed_fits = []

buzsaki_amy_failed_autocorr = []
buzsaki_amy_no_spikes_in_a_bin = []
buzsaki_amy_low_fr = []

buzsaki_amy_avg_fr = []
buzsaki_amy_correlation_matrices = []
    ...
    if unit_spikes == []:
        ...
        try:
            ...
            if np.isnan(one_autocorrelation).any() == True:
                ...
                buzsaki_amy_failed_autocorr.append(unit) # skip this unit if any autocorrelation fails
        
            elif [summed_spikes_per_bin[bin] == 0 for bin in range(len(summed_spikes_per_bin))] == True:
        
                buzsaki_amy_no_spikes_in_a_bin.append(unit) # skip this unit if any bin doesn't have spikes
        
            elif np.sum(summed_spikes_per_bin) < 1:
        
                buzsaki_amy_low_fr.append(unit) # skip this unit if avg firing rate across all trials is < 1
                ...
                buzsaki_amy_correlation_matrices.append(correlation_matrix)
        
                # plt.imshow(correlation_matrix)
                # plt.title('Rat amygdala unit %i' %unit)
                # plt.xlabel('lag')
                # plt.ylabel('lag')
                ...
                try:
                    ...
                    buzsaki_amy_failed_fits.append(unit)
                    
                r2 = (np.corrcoef(y_m[first_neg_diff:],func(x_m[first_neg_diff:],*pars)))[0,1]**2
        
                buzsaki_amy_taus.append(pars[1])
                buzsaki_amy_avg_fr.append(np.sum(summed_spikes_per_bin)/trials)
        
                buzsaki_amy_all_means.append(y_m)
        
                # plt.plot(x_m,y_m,'ro',label='original data')
                # plt.plot(x_m[first_neg_diff:],func(x_m[first_neg_diff:],*pars),label='fit')
                # plt.xlabel('lag (ms)')
                # plt.ylabel('mean autocorrelation')
                # plt.title('Rat amygdala %i' %unit)
                # plt.legend()
                # plt.show()
                
                #%% Add data to 'all_data'
                
                all_data_buzsaki_amy.append(('buzsaki','rat','bla',unit,pars[1],np.sum(summed_spikes_per_bin)/trials,r2,pars[0],pars[2]))
...
buzsaki_amy_bad_units = len(buzsaki_amy_failed_autocorr) + len(buzsaki_amy_no_spikes_in_a_bin) + len(buzsaki_amy_low_fr)

print('%i units were filtered out' %buzsaki_amy_bad_units)
print('out of %i total units' %len(spikes))

#%% Take mean of all units

buzsaki_amy_all_means = np.vstack(buzsaki_amy_all_means)

buzsaki_amy_mean = np.mean(buzsaki_amy_all_means,axis=0)
buzsaki_amy_sd = np.std(buzsaki_amy_all_means,axis=0)
buzsaki_amy_se = buzsaki_amy_sd/np.sqrt(len(buzsaki_amy_mean))

buzsaki_amy_mean_fr = np.mean(buzsaki_amy_avg_fr)

def func(x,a,tau,b):
    return a*((np.exp(-x/tau))+b)

mean_diff = np.diff(buzsaki_amy_mean)
...
buzsaki_amy_pars,cov = curve_fit(func,x_m[first_neg_mean_diff:],buzsaki_amy_mean[first_neg_mean_diff:],p0=[1,100,1],bounds=((0,np.inf)))

plt.plot(x_m,buzsaki_amy_mean,label='original data')
plt.plot(x_m[first_neg_mean_diff:],func(x_m[first_neg_mean_diff:],*buzsaki_amy_pars),label='fit curve')
plt.legend(loc='upper right')
plt.xlabel('lag (ms)')
plt.ylabel('autocorrelation')
plt.title('Mean of all Rat amygdala units \n Buzsaki asleep 2')
plt.text(710,0.12,'tau = %i ms \n fr = %.2f hz \n n = %i' % (buzsaki_amy_pars[1],buzsaki_amy_mean_fr,len(buzsaki_amy_taus)))
plt.show()


#%% Add error bars

plt.errorbar(x_m, buzsaki_amy_mean, yerr=buzsaki_amy_se, label='data +/- se')
plt.plot(x_m[first_neg_mean_diff:],func(x_m[first_neg_mean_diff:],*buzsaki_amy_pars),label='fit curve')
plt.legend(loc='upper right')
plt.xlabel('lag (ms)')
plt.ylabel('autocorrelation')
...
plt.hist(buzsaki_amy_taus,bins=bins, weights=np.zeros_like(buzsaki_amy_taus) + 1. / len(buzsaki_amy_taus))
plt.axvline(buzsaki_amy_pars[1],color='r',linestyle='dashed',linewidth=1)
plt.xlabel('tau (ms)')
plt.ylabel('proportion')
plt.xscale('log')
plt.title('%i Rat amygdala units \n Buzsaki, asleep 2' %len(buzsaki_amy_taus))
plt.show()

#%% Correlation matrix

buzsaki_amy_mean_matrix = np.mean(buzsaki_amy_correlation_matrices,axis=0)

plt.imshow(buzsaki_amy_mean_matrix)
plt.tight_layout()
plt.title('Buzsaki Rat amygdala')
plt.xlabel('lag (ms)')
plt.ylabel('lag (ms)')
