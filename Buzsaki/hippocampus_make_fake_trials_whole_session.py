...
for unit in range(len(spikes)):
    ...
    if np.isnan(one_autocorrelation).any() == True:
        ...
        # plt.title('Rat hippocampus unit %i' %unit)
        # plt.xlabel('lag')
        # plt.ylabel('lag')
        ...
        # plt.title('Rat hippocampus %i' %unit)
        # plt.legend()
        # plt.show()
        
        #%% Add data to 'all_data'
        
        all_data_buzsaki_hc.append(('buzsaki','rat','hippocampus',unit+1,pars[1],np.sum(summed_spikes_per_bin)/trials,r2,pars[0],pars[2]))

with open('/Users/zachz/Documents/timescales_analysis/results.csv','a') as out:
    csv_out=csv.writer(out)
    for row in all_data_buzsaki_hc:
