...
ila = spio.loadmat('/Users/zachz/Dropbox/Timescales across species/By trial/Steinmetz - mouse/ila_task_trial.mat',simplify_cells=True)

#%% Extract spiking data from one brain area

spikes = ila['ila_task_by_trial']

all_data_steinmetz_ila = []

steinmetz_ila_all_means = []
steinmetz_ila_taus = []
steinmetz_ila_failed_fits = []

steinmetz_ila_failed_autocorr = []
steinmetz_ila_no_spikes_in_a_bin = []
steinmetz_ila_low_fr = []

steinmetz_ila_avg_fr = []
steinmetz_ila_correlation_matrices = []
...
for unit in range(len(spikes)):
        ...
        try:
            ...
            if np.isnan(one_autocorrelation).any() == True:
                ...
                steinmetz_ila_failed_autocorr.append(unit) # skip this unit if any autocorrelation fails

            elif [summed_spikes_per_bin[bin] == 0 for bin in range(len(summed_spikes_per_bin))] == True:

                steinmetz_ila_no_spikes_in_a_bin.append(unit) # skip this unit if any bin doesn't have spikes

            elif np.sum(summed_spikes_per_bin) < 1:

                steinmetz_ila_low_fr.append(unit) # skip this unit if avg firing rate across all trials is < 1
                ...
                steinmetz_ila_correlation_matrices.append(correlation_matrix)

                # plt.imshow(correlation_matrix)
                # plt.title('Mouse ila unit %i' %unit)
                # plt.xlabel('lag')
                # plt.ylabel('lag')
                ...
                try:
                    ...
                    steinmetz_ila_failed_fits.append(unit)

                r2 = (np.corrcoef(y_m[first_neg_diff:],func(x_m[first_neg_diff:],*pars)))[0,1]**2

                steinmetz_ila_taus.append(pars[1])
                steinmetz_ila_avg_fr.append(np.sum(summed_spikes_per_bin)/trials)

                steinmetz_ila_all_means.append(y_m)

                # plt.plot(x_m,y_m,'ro',label='original data')
                # plt.plot(x_m[first_neg_diff:],func(x_m[first_neg_diff:],*pars),label='fit')
                # plt.xlabel('lag (ms)')
                # plt.ylabel('mean autocorrelation')
                # plt.title('Mouse ila %i' %unit)
                # plt.legend()
                # plt.show()

                #%% Add data to 'all_data'

                all_data_steinmetz_ila.append(('steinmetz','mouse','ila',unit,pars[1],np.sum(summed_spikes_per_bin)/trials,r2,pars[0],pars[2]))
...
with open('/Users/zachz/Documents/timescales_analysis/results.csv','a') as out:
    ...
    for row in all_data_steinmetz_ila:
...
print('%i units were filtered out' %steinmetz_ila_bad_units)
print('out of %i total units' %len(spikes))

#%% Take mean of all units

steinmetz_ila_all_means = np.vstack(steinmetz_ila_all_means)

steinmetz_ila_mean = np.mean(steinmetz_ila_all_means,axis=0)
...
mean_diff = np.diff(steinmetz_ila_mean)
...
steinmetz_ila_pars,cov = curve_fit(func,x_m[first_neg_mean_diff:],steinmetz_ila_mean[first_neg_mean_diff:],p0=[1,100,1],bounds=((0,np.inf)))

plt.plot(x_m,steinmetz_ila_mean,label='original data')
...
a_population_steinmetz_ila = (('steinmetz','ila',steinmetz_ila_pars[1],steinmetz_ila_mean_fr,len(steinmetz_ila_taus)))
...
steinmetz_ila_mean_matrix = np.mean(steinmetz_ila_correlation_matrices,axis=0)
