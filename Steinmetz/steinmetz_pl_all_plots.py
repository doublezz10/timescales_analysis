...
all_data_steinmetz_pl = []

steinmetz_pl_all_means = []
steinmetz_pl_taus = []
steinmetz_pl_failed_fits = []

steinmetz_pl_failed_autocorr = []
steinmetz_pl_no_spikes_in_a_bin = []
steinmetz_pl_low_fr = []

steinmetz_pl_avg_fr = []
steinmetz_pl_correlation_matrices = []
...
for unit in range(len(spikes)):
        ...
        try:
            ...
            if np.isnan(one_autocorrelation).any() == True:
                ...
                steinmetz_pl_failed_autocorr.append(unit) # skip this unit if any autocorrelation fails

            elif [summed_spikes_per_bin[bin] == 0 for bin in range(len(summed_spikes_per_bin))] == True:

                steinmetz_pl_no_spikes_in_a_bin.append(unit) # skip this unit if any bin doesn't have spikes

            elif np.sum(summed_spikes_per_bin) < 1:

                steinmetz_pl_low_fr.append(unit) # skip this unit if avg firing rate across all trials is < 1
                ...
                steinmetz_pl_correlation_matrices.append(correlation_matrix)

                # plt.imshow(correlation_matrix)
                # plt.title('Mouse pl unit %i' %unit)
                # plt.xlabel('lag')
                # plt.ylabel('lag')
                ...
                try:
                    ...
                    steinmetz_pl_failed_fits.append(unit)

                r2 = (np.corrcoef(y_m[first_neg_diff:],func(x_m[first_neg_diff:],*pars)))[0,1]**2

                steinmetz_pl_taus.append(pars[1])
                steinmetz_pl_avg_fr.append(np.sum(summed_spikes_per_bin)/trials)

                steinmetz_pl_all_means.append(y_m)

                # plt.plot(x_m,y_m,'ro',label='original data')
                # plt.plot(x_m[first_neg_diff:],func(x_m[first_neg_diff:],*pars),label='fit')
                # plt.xlabel('lag (ms)')
                # plt.ylabel('mean autocorrelation')
                # plt.title('Mouse pl %i' %unit)
                # plt.legend()
                # plt.show()

                #%% Add data to 'all_data'

                all_data_steinmetz_pl.append(('steinmetz','mosue','pl',unit,pars[1],np.sum(summed_spikes_per_bin)/trials,r2,pars[0],pars[2]))
...
with open('/Users/zachz/Documents/timescales_analysis/results.csv','a') as out:
    ...
    for row in all_data_steinmetz_pl:
        csv_out.writerow(row)
        
#%% How many units got filtered?

steinmetz_pl_bad_units = len(steinmetz_pl_failed_autocorr) + len(steinmetz_pl_no_spikes_in_a_bin) + len(steinmetz_pl_low_fr)

print('%i units were filtered out' %steinmetz_pl_bad_units)
print('out of %i total units' %len(spikes))

#%% Take mean of all units

steinmetz_pl_all_means = np.vstack(steinmetz_pl_all_means)

steinmetz_pl_mean = np.mean(steinmetz_pl_all_means,axis=0)
...
mean_diff = np.diff(steinmetz_pl_mean)
...
steinmetz_pl_pars,cov = curve_fit(func,x_m[first_neg_mean_diff:],steinmetz_pl_mean[first_neg_mean_diff:],p0=[1,100,1],bounds=((0,np.inf)))

plt.plot(x_m,steinmetz_pl_mean,label='original data')
...
a_population_steinmetz_pl = (('steinmetz','pl',steinmetz_pl_pars[1],steinmetz_pl_mean_fr,len(steinmetz_pl_taus)))
...
plt.hist(steinmetz_pl_taus,bins=bins, weights=np.zeros_like(steinmetz_pl_taus) + 1. / len(steinmetz_pl_taus))
plt.axvline(steinmetz_pl_pars[1],color='r',linestyle='dashed',linewidth=1)
plt.xlabel('tau (ms)')
plt.ylabel('proportion')
plt.xscale('log')
plt.title('%i Mouse PL units \n Steinmetz' %len(steinmetz_pl_taus))
plt.show()

#%% Correlation matrix

steinmetz_pl_mean_matrix = np.mean(steinmetz_pl_correlation_matrices,axis=0)

plt.imshow(steinmetz_pl_mean_matrix)
plt.tight_layout()
plt.title('Steinmetz PL')
plt.xlabel('lag (ms)')
plt.ylabel('lag (ms)')
