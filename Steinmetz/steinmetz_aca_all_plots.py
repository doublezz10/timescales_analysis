...
all_data_steinmetz_aca = []

steinmetz_aca_all_means = []
steinmetz_aca_taus = []
steinmetz_aca_failed_fits = []

steinmetz_aca_failed_autocorr = []
steinmetz_aca_no_spikes_in_a_bin = []
steinmetz_aca_low_fr = []

steinmetz_aca_avg_fr = []
steinmetz_aca_correlation_matrices = []
...
for unit in range(len(spikes)):
        ...
        try:
            ...
            if np.isnan(one_autocorrelation).any() == True:
                ...
                steinmetz_aca_failed_autocorr.append(unit) # skip this unit if any autocorrelation fails

            elif [summed_spikes_per_bin[bin] == 0 for bin in range(len(summed_spikes_per_bin))] == True:

                steinmetz_aca_no_spikes_in_a_bin.append(unit) # skip this unit if any bin doesn't have spikes

            elif np.sum(summed_spikes_per_bin) < 1:

                steinmetz_aca_low_fr.append(unit) # skip this unit if avg firing rate across all trials is < 1
                ...
                steinmetz_aca_correlation_matrices.append(correlation_matrix)

                # plt.imshow(correlation_matrix)
                # plt.title('Mouse ACA unit %i' %unit)
                # plt.xlabel('lag')
                # plt.ylabel('lag')
                ...
                try:
                    ...
                    steinmetz_aca_failed_fits.append(unit)

                r2 = (np.corrcoef(y_m[first_neg_diff:],func(x_m[first_neg_diff:],*pars)))[0,1]**2

                steinmetz_aca_taus.append(pars[1])
                steinmetz_aca_avg_fr.append(np.sum(summed_spikes_per_bin)/trials)

                steinmetz_aca_all_means.append(y_m)

                # plt.plot(x_m,y_m,'ro',label='original data')
                # plt.plot(x_m[first_neg_diff:],func(x_m[first_neg_diff:],*pars),label='fit')
                # plt.xlabel('lag (ms)')
                # plt.ylabel('mean autocorrelation')
                # plt.title('Mouse ACA %i' %unit)
                # plt.legend()
                # plt.show()

                #%% Add data to 'all_data'

                all_data_steinmetz_aca.append(('steinmetz','mouse','aca',unit,pars[1],np.sum(summed_spikes_per_bin)/trials,r2,pars[0],pars[2]))
...
with open('/Users/zachz/Documents/timescales_analysis/results.csv','a') as out:
    ...
    for row in all_data_steinmetz_aca:
        csv_out.writerow(row)
#%% How many units got filtered?

steinmetz_aca_bad_units = len(steinmetz_aca_failed_autocorr) + len(steinmetz_aca_no_spikes_in_a_bin) + len(steinmetz_aca_low_fr)

print('%i units were filtered out' %steinmetz_aca_bad_units)
print('out of %i total units' %len(spikes))

#%% Take mean of all units

steinmetz_aca_all_means = np.vstack(steinmetz_aca_all_means)

steinmetz_aca_mean = np.mean(steinmetz_aca_all_means,axis=0)
...
mean_diff = np.diff(steinmetz_aca_mean)
...
steinmetz_aca_pars,cov = curve_fit(func,x_m[first_neg_mean_diff:],steinmetz_aca_mean[first_neg_mean_diff:],p0=[1,100,1],bounds=((0,np.inf)))

plt.plot(x_m,steinmetz_aca_mean,label='original data')
plt.plot(x_m[first_neg_mean_diff:],func(x_m[first_neg_mean_diff:],*steinmetz_aca_pars),label='fit curve')
...
a_population_steinmetz_aca = (('steinmetz','aca',steinmetz_aca_pars[1],steinmetz_aca_mean_fr,len(steinmetz_aca_taus)))

#%% Add error bars

plt.errorbar(x_m, steinmetz_aca_mean, yerr=steinmetz_aca_se, label='data +/- se')
plt.plot(x_m[first_neg_mean_diff:],func(x_m[first_neg_mean_diff:],*steinmetz_aca_pars),label='fit curve')
plt.legend(loc='upper right')
plt.xlabel('lag (ms)')
plt.ylabel('autocorrelation')
...
plt.hist(steinmetz_aca_taus,bins=bins, weights=np.zeros_like(steinmetz_aca_taus) + 1. / len(steinmetz_aca_taus))
plt.axvline(steinmetz_aca_pars[1],color='r',linestyle='dashed',linewidth=1)
plt.xlabel('tau (ms)')
plt.ylabel('proportion')
plt.xscale('log')
plt.title('%i Mouse ACA units \n Steinmetz' %len(steinmetz_aca_taus))
plt.show()
#%% Correlation matrix

steinmetz_aca_mean_matrix = np.mean(steinmetz_aca_correlation_matrices,axis=0)

plt.imshow(steinmetz_aca_mean_matrix)
plt.tight_layout()
plt.title('Steinmetz ACA')
plt.xlabel('lag (ms)')
plt.ylabel('lag (ms)')
