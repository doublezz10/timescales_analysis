...
orb = spio.loadmat('/Users/zachz/Dropbox/Timescales across species/By trial/Steinmetz - mouse/orb_task_trial.mat',simplify_cells=True)

#%% Extract spiking data from one brain area

spikes = orb['orb_task_by_trial']

all_data_steinmetz_orb = []

steinmetz_orb_all_means = []
steinmetz_orb_taus = []
steinmetz_orb_failed_fits = []

steinmetz_orb_failed_autocorr = []
steinmetz_orb_no_spikes_in_a_bin = []
steinmetz_orb_low_fr = []

steinmetz_orb_avg_fr = []
steinmetz_orb_correlation_matrices = []
...
for unit in range(len(spikes)):
        ...
        try:
            ...
            if np.isnan(one_autocorrelation).any() == True:
                ...
                steinmetz_orb_failed_autocorr.append(unit) # skip this unit if any autocorrelation fails

            elif [summed_spikes_per_bin[bin] == 0 for bin in range(len(summed_spikes_per_bin))] == True:

                steinmetz_orb_no_spikes_in_a_bin.append(unit) # skip this unit if any bin doesn't have spikes

            elif np.sum(summed_spikes_per_bin) < 1:

                steinmetz_orb_low_fr.append(unit) # skip this unit if avg firing rate across all trials is < 1
                ...
                steinmetz_orb_correlation_matrices.append(correlation_matrix)

                # plt.imshow(correlation_matrix)
                # plt.title('Mouse orb unit %i' %unit)
                # plt.xlabel('lag')
                # plt.ylabel('lag')
                ...
                try:
                    ...
                    steinmetz_orb_failed_fits.append(unit)

                r2 = (np.corrcoef(y_m[first_neg_diff:],func(x_m[first_neg_diff:],*pars)))[0,1]**2

                steinmetz_orb_taus.append(pars[1])
                steinmetz_orb_avg_fr.append(np.sum(summed_spikes_per_bin)/trials)

                steinmetz_orb_all_means.append(y_m)

                # plt.plot(x_m,y_m,'ro',label='original data')
                # plt.plot(x_m[first_neg_diff:],func(x_m[first_neg_diff:],*pars),label='fit')
                # plt.xlabel('lag (ms)')
                # plt.ylabel('mean autocorrelation')
                # plt.title('Mouse orb %i' %unit)
                # plt.legend()
                # plt.show()

                #%% Add data to 'all_data'

                all_data_steinmetz_orb.append(('steinmetz','mouse','orb',unit,pars[1],np.sum(summed_spikes_per_bin)/trials,r2,pars[0],pars[2]))
...
with open('/Users/zachz/Documents/timescales_analysis/results.csv','a') as out:
    ...
    for row in all_data_steinmetz_orb:
        csv_out.writerow(row)
        
#%% How many units got filtered?

steinmetz_orb_bad_units = len(steinmetz_orb_failed_autocorr) + len(steinmetz_orb_no_spikes_in_a_bin) + len(steinmetz_orb_low_fr)

print('%i units were filtered out' %steinmetz_orb_bad_units)
print('out of %i total units' %len(spikes))

#%% Take mean of all units

steinmetz_orb_all_means = np.vstack(steinmetz_orb_all_means)

steinmetz_orb_mean = np.mean(steinmetz_orb_all_means,axis=0)
steinmetz_orb_sd = np.std(steinmetz_orb_all_means,axis=0)
steinmetz_orb_se = steinmetz_orb_sd/np.sqrt(len(steinmetz_orb_mean))

steinmetz_orb_mean_fr = np.mean(steinmetz_orb_avg_fr)

def func(x,a,tau,b):
    return a*((np.exp(-x/tau))+b)

mean_diff = np.diff(steinmetz_orb_mean)
...
steinmetz_orb_pars,cov = curve_fit(func,x_m[first_neg_mean_diff:],steinmetz_orb_mean[first_neg_mean_diff:],p0=[1,100,1],bounds=((0,np.inf)))

plt.plot(x_m,steinmetz_orb_mean,label='original data')
plt.plot(x_m[first_neg_mean_diff:],func(x_m[first_neg_mean_diff:],*steinmetz_orb_pars),label='fit curve')
plt.legend(loc='upper right')
plt.xlabel('lag (ms)')
plt.ylabel('mean autocorrelation')
plt.title('Mean of all mouse ORB units \n Steinmetz')
plt.text(710,0.08,'tau = %i ms \n fr = %.2f hz \n n = %i' % (steinmetz_orb_pars[1],steinmetz_orb_mean_fr,len(steinmetz_orb_taus)))
plt.show()

a_population_steinmetz_orb = (('steinmetz','orb',steinmetz_orb_pars[1],steinmetz_orb_mean_fr,len(steinmetz_orb_taus)))

#%% Add error bars

plt.errorbar(x_m, steinmetz_orb_mean, yerr=steinmetz_orb_se, label='data +/- se')
plt.plot(x_m[first_neg_mean_diff:],func(x_m[first_neg_mean_diff:],*steinmetz_orb_pars),label='fit curve')
plt.legend(loc='upper right')
plt.xlabel('lag (ms)')
plt.ylabel('autocorrelation')
...
plt.hist(steinmetz_orb_taus,bins=bins, weights=np.zeros_like(steinmetz_orb_taus) + 1. / len(steinmetz_orb_taus))
plt.axvline(steinmetz_orb_pars[1],color='r',linestyle='dashed',linewidth=1)
plt.xlabel('tau (ms)')
plt.ylabel('proportion')
plt.xscale('log')
plt.title('%i Mouse OFC units \n Steinmetz' %len(steinmetz_orb_taus))
plt.show()

#%% Correlation matrix

steinmetz_orb_mean_matrix = np.mean(steinmetz_orb_correlation_matrices,axis=0)

plt.imshow(steinmetz_orb_mean_matrix)
plt.tight_layout()
