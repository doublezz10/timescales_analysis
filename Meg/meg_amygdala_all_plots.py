...
for unit in range(len(spikes)):
        ...
        if np.isnan(one_autocorrelation).any() == True:
            ...
            all_data_meg_amyg.append(('meg','monkey','amygdala',unit,pars[1],np.sum(summed_spikes_per_bin)/trials,r2,pars[0],pars[2]))

with open('/Users/zachz/Documents/timescales_analysis/results.csv','a') as out:
    csv_out=csv.writer(out)
    for row in all_data_meg_amyg:
...
a_population_meg_amyg = (('meg','amygdala',meg_amyg_pars[1],meg_amyg_mean_fr,len(meg_amyg_taus)))
