...
for unit in range(len(spikes)):
        ...
        if np.isnan(one_autocorrelation).any() == True:
            ...
            all_data_meg_sc.append(('meg','monkey','scACC',unit,pars[1],np.sum(summed_spikes_per_bin)/trials,r2,pars[0],pars[2]))

with open('/Users/zachz/Documents/timescales_analysis/results.csv','a') as out:
    csv_out=csv.writer(out)
    for row in all_data_meg_sc:
...
a_population_meg_sc = (('meg','scACC',meg_sc_pars[1],meg_sc_mean_fr,len(meg_sc_taus)))
