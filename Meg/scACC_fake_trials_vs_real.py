...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb 18 16:33:52 2021
...
real_trials = pd.DataFrame(all_data_meg_sc,columns=['dataset','species','brain_area','unit_id','tau','fr','r2','a','b'])
filtered_real = real_trials[real_trials['r2'] >= 0.8]
filtered_real = real_trials[real_trials['tau'] <= 1000]
fake_trials = pd.DataFrame(all_data_meg_sc_faketrials,columns=['dataset','species','brain_area','unit_id','tau','fr','r2','a','b'])
filtered_fake = fake_trials[fake_trials['r2'] >= 0.8]
...
plt.xlabel('real trials')
plt.ylabel('fake trials')
plt.title('scACC filtered for R$^2$ > 0.8')
plt.text(0,800,'R$^2$ = %.2f' %r2)
plt.legend()
plt.show()
