...
for unit in range(len(spikes)):
        ...
        if np.isnan(one_autocorrelation).any() == True:
            ...
            all_data_meg_vs.append(('meg','monkey','ventralStriatum',unit,pars[1],np.sum(summed_spikes_per_bin)/trials,r2,pars[0],pars[2]))

with open('/Users/zachz/Documents/timescales_analysis/results.csv','a') as out:
    csv_out=csv.writer(out)
    for row in all_data_meg_vs:
...
meg_vs_pars,cov = curve_fit(func,x_m[first_neg_mean_diff:],meg_vs_mean[first_neg_mean_diff:],p0=[1,100,1],bounds=((0,np.inf)))

plt.plot(x_m,meg_vs_mean,label='original data')
plt.plot(x_m[first_neg_mean_diff:],func(x_m[first_neg_mean_diff:],*meg_vs_pars),label='fit curve')
...
a_population_meg_vs = (('meg','ventralStriatum',meg_vs_pars[1],meg_vs_mean_fr,len(meg_vs_taus)))

#%% Histogram of taus

bins = 10**np.arange(0,4,0.1)

plt.hist(meg_vs_taus,bins=bins, weights=np.zeros_like(meg_vs_taus) + 1. / len(meg_vs_taus))
plt.axvline(meg_vs_pars[1],color='r',linestyle='dashed',linewidth=1)
plt.xlabel('tau (ms)')
plt.ylabel('proportion')
plt.xscale('log')
plt.title('%i Human vStriatum units \n Meg' %len(meg_vs_taus))
plt.show()
