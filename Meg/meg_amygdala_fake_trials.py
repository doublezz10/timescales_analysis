...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb 18 11:53:16 2021
...
amy = spio.loadmat('~/Meg - monkey/meg_amygdala.mat',simplify_cells=True)
...
for run in range(100):
    ...
    for unit in range(len(spikes)):
        ...
        if np.isnan(one_autocorrelation).any() == True:
            ...
            meg_amyg_no_spikes_in_a_bin.append(unit) # skip this unit if any bin doesn't have spikes
            ...
            all_data_meg_amyg_faketrials.append(('meg','monkey','amyg',run,unit,pars[1],np.sum(summed_spikes_per_bin)/trials,r2,pars[0],pars[2]))

with open('~/meg_amyg_results.csv','w') as out:
    csv_out=csv.writer(out)
    csv_out.writerow(['dataset','species','brain_area','iteration','unit','tau','fr','r2','a','b'])
    for row in all_data_meg_amyg_faketrials:
...
# plt.xscale('log')
# plt.title('%i Monkey amygdala units \n Meg (fake trials)' %len(meg_amyg_taus))
# plt.show()
